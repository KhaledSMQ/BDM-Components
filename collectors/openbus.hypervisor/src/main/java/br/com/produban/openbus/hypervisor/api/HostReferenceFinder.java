...
public class HostReferenceFinder {
    ...
    public Map<String, ManagedObjectReference> findAllHost() throws RuntimeFaultFaultMsg, InvalidPropertyFaultMsg {

	return findAllHosts(connection.getServiceContent().getRootFolder(), "HostSystem", new RetrieveOptions());
    }

    public Map<String, ManagedObjectReference> findAllVms() throws RuntimeFaultFaultMsg, InvalidPropertyFaultMsg {

	return findAllHosts(connection.getServiceContent().getRootFolder(), "VirtualMachine", new RetrieveOptions());
    }

    public Map<String, ManagedObjectReference> findAllCluster() throws RuntimeFaultFaultMsg, InvalidPropertyFaultMsg {

	return findAllHosts(connection.getServiceContent().getRootFolder(), "ClusterComputeResource", new RetrieveOptions());
    }
	...
	RetrieveResult results = connection.getVimPort().retrievePropertiesEx(connection.getPropertyCollector(),
	...
	while (results != null && !results.getObjects().isEmpty()) {
	    ...
	    results = (token != null)
		    ? connection.getVimPort().continueRetrievePropertiesEx(connection.getPropertyCollector(), token)
		    : null;
	}
    ...
    private PropertyFilterSpec[] propertyFilterSpecs(ManagedObjectReference container, String morefType,
	...
	ManagedObjectReference containerView = connection.getVimPort().createContainerView(viewManager, container,
