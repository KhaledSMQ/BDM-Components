...
public class MQAgent {
	...
	static final String copyright_notice = "Licensed Materials - Property of IBM " + "5724-H72, 5655-R36, 5724-L26, 5655-L82                "
			+ "(c) Copyright IBM Corp. 2008, 2009 All Rights Reserved. " + "US Government Users Restricted Rights - Use, duplication or " + "disclosure restricted by GSA ADP Schedule Contract with "
			+ "IBM Corp.";
	// @COPYRIGHT_END@

	/** The SCCSID which is expanded when the file is extracted from CMVC */
	public static final String sccsid = "@(#) samples/pcf/PCF_CommonMethods.java, jmscc.samples, k710, k710-002-120928 1.11.2.1 11/10/17 15:58:48"; //$NON-NLS-1$
	...
	public static final String pcfChannel = "CHANNEL1";
	/**
	 ...
	 */
	// public String channel = "SVRCONN";
	public String channel = "CHANNEL1";
	...
	public void CreateAgent(int numOfArgs) throws MQDataException {
		try {
			if (numOfArgs == 1) {
				...
				// Connect to the client and define the queue manager host, port
				// and channel.
				// Notice that the method does not take a queue manager name. It
			...
			if (mqde.reasonCode == CMQC.MQRC_Q_MGR_NAME_ERROR) {
				...
				System.out.println(" or could not find the default channel \"" + channel + "\" on the queue manager.");
			}
	...
	public void DisplayException(Exception exception) {
		if (exception.getClass().equals(PCFException.class)) {
			...
			if (mqe.reasonCode == CMQC.MQRC_UNKNOWN_OBJECT_NAME) {
				System.out.println("Cound not find the queue \"" + pcfQueue + "\" on the queue manager \"" + queueManager + "\".");
			} else {
	...
	public MQAgent createAgent(String hostname, String port) throws MQDataException {
		...
		MQAgent commonMethods = new MQAgent();

		if (!commonMethods.ParseParameters(args)) {
			return null;
		}

		commonMethods.CreateAgent(args.length);
