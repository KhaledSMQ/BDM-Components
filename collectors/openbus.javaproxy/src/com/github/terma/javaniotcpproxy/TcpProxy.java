...
public class TcpProxy {
    ...
    public TcpProxy(final TcpProxyConfig config, IntegrationQueue integrationQueue) {
        TcpProxyConnectorFactory handlerFactory = new TcpProxyConnectorFactory(config, integrationQueue);

        final TcpServerConfig serverConfig =
                new TcpServerConfig(config.getLocalPort(), handlerFactory,
                                    config.getWorkerCount(), config.getIdlenessTickInterval(),
                                    config.getIdleClientTimeout());

        server = new TcpServer(serverConfig, integrationQueue);
    }
