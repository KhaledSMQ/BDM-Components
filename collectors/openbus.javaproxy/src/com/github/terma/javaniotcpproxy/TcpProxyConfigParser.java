...
public class TcpProxyConfigParser {
    ...
    public static List<TcpProxyConfig> parse(Properties properties) {
        ...
        for (final String proxyName : proxyNames) {
            ...
            final String topicName = findProperty(properties, proxyName + ".topicName");

            TcpProxyConfig config = new TcpProxyConfig(localPort, remoteHost, remotePort, onlyActiveChecks, topicName);
            config.setWorkerCount(findIntegerProperty(properties, proxyName + ".workerCount"));
            config.setIdlenessTickInterval(findIntegerProperty(properties, proxyName + ".idlenessTickInterval"));
            config.setIdleClientTimeout(findIntegerProperty(properties, proxyName + ".idleClientTimeout"));

            tcpProxyConfigs.add(config);
        }
